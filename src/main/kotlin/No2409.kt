...
class No2409 {
    ...
    /*
    *执行耗时:152 ms,击败了100.00% 的Kotlin用户
	*内存消耗:34.4 MB,击败了100.00% 的Kotlin用户
    * */

    fun countDaysTogether(arriveAlice: String, leaveAlice: String, arriveBob: String, leaveBob: String): Int {
        val ints = arrayOf(0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31)
        for (index in 1 until ints.size) {
            ints[index] = ints[index] + ints[index - 1]
        }
        //得到天数 一年中的多少天
        val aA = ints[arriveAlice.split("-")[0].toInt() - 1] + arriveAlice.split("-")[1].toInt()
        val aL = ints[leaveAlice.split("-")[0].toInt() - 1] + leaveAlice.split("-")[1].toInt()
        val bA = ints[arriveBob.split("-")[0].toInt() - 1] + arriveBob.split("-")[1].toInt()
        val bL = ints[leaveBob.split("-")[0].toInt() - 1] + leaveBob.split("-")[1].toInt()
        val days = Math.min(bL, aL) - Math.max(aA, bA)
        return if (days < 0) {
            //特殊情况处理 不同时在
            0
        } else if (days == 0) {
...
fun main() {
    var countDaysTogether = No2409().countDaysTogether("08-15", "08-18", "08-16", "08-16")
    println(countDaysTogether)
}